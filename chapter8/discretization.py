...
if __name__ == '__main__': #判断是否主窗口运行，如果是将代码保存为.py后运行，则需要这句，如果直接复制到命令窗口运行，则不需要这句。
  for i in range(len(keys)):
    #调用k-means算法，进行聚类离散化
    print(u'正在进行“%s”这列的聚类...' % keys[i])
    kmodel = KMeans(n_clusters = k, n_jobs = 4) #n_jobs是并行数，一般等于CPU数较好
    temp0 =[keys[i]]
    temp1 = data[temp0]
    temp2 = temp1.as_matrix() 
    kmodel.fit(temp2) #训练模型
    
    r1 = pd.DataFrame(kmodel.cluster_centers_, columns = [typelabel[keys[i]]]) #聚类中心
    r2 = pd.Series(kmodel.labels_).value_counts() #分类统计
    r2 = pd.DataFrame(r2, columns = [typelabel[keys[i]]+'n']) #转为DataFrame，记录各个类别的数目
    temp = pd.concat([r1, r2], axis = 1) #匹配聚类中心和类别数目 ，concat是两个DataFrame里的Series进行列的并排搞到一起
    r = temp.sort(typelabel[keys[i]])#把head等于typelabel[keys[i]的列排序
    r.index = [1, 2, 3, 4]
